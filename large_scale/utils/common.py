...
def save_checkpoint(state, epoch, checkpoint='none', filename='checkpoint.pth.tar', neptune_run=None):
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if neptune_run != None:
        neptune_run["model/saved_model"].upload(filepath)
        ...
        if args.epochs == 200:
            if True:
                ...
                if epoch <= args.warmup:
                    ...
                    lr = args.lr * cos_decay(epoch=epoch, begin=args.warmup, end=args.epochs)
